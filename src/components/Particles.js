...
export function Particles({ speed, fov, aperture, focus, curl, size = 500, ...props }) {
	const simRef = useRef()
	...
	useFrame((state) => {
		...
		const newFocus = Math.sin(state.clock.elapsedTime / 10) + 6
		renderRef.current.uniforms.uFocus.value = newFocus
