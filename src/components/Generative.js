...
function Fatline({ curve, width, color, speed }) {
	...
	return (
		<mesh>
			...
			<meshLineMaterial ref={material} attach="material" transparent depthTest={false} lineWidth={width} color={color} dashArray={0.5} dashRatio={0.4} />
		</mesh>
	)
}
		...
		() =>
			new Array(count).fill().map(() => {
				...
				const points = new Array(30).fill().map((_, index) => {
					...
					return pos.add(new THREE.Vector3(Math.sin(angle) * radius * r(), Math.cos(angle) * radius * r())).clone()
				})
...
function Generative() {
	...
	const onMouseMove = ({ pageX: x, pageY: y }) => (mouse.current = [x - window.innerWidth / 2, y - window.innerHeight / 2])
	...
	return (
		<div onMouseMove={onMouseMove}>
			<Canvas
				...
				<Lines count={10} mouse={mouse} colors={['#4E3A95', '#FAD1C9', '#18225E', '#E079C3', '#665F80']} />
				<Effect />
			</Canvas>
			<style jsx>{`
      div {
